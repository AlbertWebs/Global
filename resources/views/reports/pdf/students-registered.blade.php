...
<head>
    <meta charset="utf-8">
    <title>Students Registered Report</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="header">
        ...
        <div class="subtitle">Students Registered Report</div>
        <div class="period">
            @if($dateFrom && $dateTo)
    ...
    <div class="table-container">
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td class="admission-number">{{ $student->admission_number ?? 'N/A' }}</td>
                    <td class="full-name">{{ $student->full_name }}</td>
                    <td class="phone-number">{{ $student->phone ?? 'N/A' }}</td>
                    <td class="registration-date">{{ $student->created_at->format('F d, Y') }}</td>
                </tr>
                @endforeach
                <tr class="total-row">
                    <td colspan="3" class="text-right" style="font-size: 12px;">TOTAL STUDENTS:</td>
                    <td class="text-center" style="font-size: 12px;">{{ $students->count() }}</td>
                </tr>
            </tbody>
        </table>
    </div>
